...
def get_parser(parser=None):
    ...
    parser.add_argument("--dump-write", action="store_true", default=False, help="Writes intermediate map every 10 iterations")
    return parser

### Settting up Report Logger  ###
...
def error_log_header(odir):
    with open(os.path.join(odir,"error_log.log"), "a") as error_file:
        error_file.write("\n")
        error_file.write("="*40 + "\n")
        error_file.write(f"Log started at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        #error_file.write("Additional run information.\n")
        error_file.write("="*40 + "\n")
        error_file.write("\n")
...
def main(config_file=None, defaults=defaults, **args):
    ...
    for ind in range(comm.rank, len(ids), comm.size):
        ...
        for detset in detsets:
            ...
            #print(obis.glitch_flags)
            print(obs)
            #print(help(sotodlib.tod_ops.flags.get_det_bias_flags))
            ta, left, right = sotodlib.tod_ops.flags.get_turnaround_flags(obs, merge=False, merge_lr= False, overwrite=True)
            #obs.wrap_new('turnaround', shape=('dets', 'samps'),ta_exp)
            #calc_aman = core.AxisManager(aman.dets, aman.samps)
            obs.wrap('turnarounds', ta, [(0, 'dets'), (1, 'samps')])
            obs.wrap('left_scan', left, [(0, 'dets'), (1, 'samps')])
            obs.wrap('right_scan', right, [(0, 'dets'), (1, 'samps')])

            print(obs)
            tod_ops.sub_polyf.subscan_polyfilter(obs, degree=1, exclude_turnarounds=True)
            exit(0)
            #tod_ops.detrend_tod(obs, method='median', count=50)
            #tod_ops.get_azss(obs, bins=10, subtract_in_place = True)
            #tod_plot(obs); exit(0)
            ...
            try:
                ...
                with mapmaking.mark("add_obs %s" % name):
                    mapmaker.add_obs(name, obs, noise_model=nmat)
                #L.debug("Successfully added %s" % (name))
                del obs
                nset_kept += 1
                ...
                with open(os.path.join(odir,"error_log.log"), "a") as error_file:
                    error_file.write(f"Failed:\t{name}\n")
                # Optionally, continue to the next detset/observation
                # without incrementing 'nset_kept'i
                del obs
                continue
    ...
    for step in mapmaker.solve(maxiter=args['maxiter']):
        ...
        if args['dump-write']:
            ...
            L.info("CG step %4d %15.7e %8.3f" % (step.i, step.err, t2-t1))
        t1 = time.time()

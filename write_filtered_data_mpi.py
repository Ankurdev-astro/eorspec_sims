...
filt_suffix = "filtchain"
obs_dir_h5 = "ccat_datacenter_mock/data_COSMOSf351_upd"
...
def process_directory(h5dir_name):
    ...
    if os.path.isdir(h5dir_path):
        print("\n")
        print(f"Rank {rank} Loading h5 dir: {h5dir_path} ...")
    ...
    for h5file in os.listdir(h5dir_path):
        ...
        if os.path.isfile(h5file_path) and h5file.endswith(".h5"):
            ...
            data.obs.append(obs)
    print(f"Rank {rank} Number of Observations Loaded: {len(data.obs)}")
    print(f"Rank {rank} Begins Filter Chain on {h5dir_name} ...")
    filter_chain(data)
    ...
    print(f"Rank {rank} Writing filtered h5 files to: {save_dir}")
...
# Distribute the directories among the available processes
for i, h5dir_name in enumerate(h5dir_names):
    if i % procs == rank:
        process_directory(h5dir_name)
